...
public class TypeConverter : MonoBehaviour
{
    ...
    static public void Prop_SetDropdown(TMP_Dropdown _d, string[] _options, int _selectedVal = 0)
    {
        //_d.ClearOptions();
        List<TMP_Dropdown.OptionData> _ol = new();
        for (int _i = 0; _i < _options.Length; _i++)
    ...
    static public void Prop_SetChildren(GameObject _prop, List<GameObject> _children, int _selectedChild)
    {
        TMP_Dropdown _d = Child_GetSelectDrop(_prop).GetComponent<TMP_Dropdown>();
        string[] _s = new string[_children.Count];
        for (int _i = 0; _i < _s.Length; _i++) _s[_i] = _children[_i].name;
        Prop_SetDropdown(_d, _s, _selectedChild);
    ...
    static public string Child_GetName(GameObject _prop)
    {
